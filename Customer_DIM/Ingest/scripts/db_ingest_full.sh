...
echo "sqoop import --connect jdbc:mysql://${mysql_host}/${mysql_db} --username ${mysql_user} --password XXXXX --table ${tables} --target-dir ${HDFS_ING_STAGE}/$tables --outdir ${OUTPUT_PATH} -m 1 --bindir ./" >> ${LOG_PATH}/log_${DT}
sqoop import --connect jdbc:mysql://${mysql_host}/${mysql_db} --username ${mysql_user} --password ${mysql_pwd} --table ${tables} --target-dir ${HDFS_ING_STAGE}/$tables --outdir ${OUTPUT_PATH} -m 1 --bindir ./ 
echo "###############################################################################" >> ${LOG_PATH}/log_${DT}
